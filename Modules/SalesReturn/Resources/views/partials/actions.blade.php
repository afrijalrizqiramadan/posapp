    ...
    <div class="dropdown-menu">
        @can('access_sale_payments')
            <a href="{{ route('sale-return-payments.index', $data->id) }}" class="dropdown-item">
                <i class="bi bi-cash-coin mr-2 text-warning" style="line-height: 1;"></i> Tampil Pembayaran
            </a>
        @endcan
        @can('access_sale_payments')
        ...
        @can('delete_sales')
            <button id="delete" class="dropdown-item"
                ...
                <form id="destroy{{ $data->id }}" class="d-none" action="{{ route('sale-returns.destroy', $data->id) }}"
                    method="POST">
                    @csrf
                    @method('delete')
                </form>
